...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    sentIndices = [tokens[word] for word in sentence]
    sentVector = np.average(wordVectors[sentIndices,:], axis = 0)
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    lossGradient = prob
    lossGradient[np.arange(N), labels] -= 1
    lossGradient /= N
    
    grad = np.dot(features.T, lossGradient)
